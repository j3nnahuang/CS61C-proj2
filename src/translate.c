...
int li_expansion(char** args, FILE* output) {
    // Check if can fit in imm field of an addiu instruction
    int failure;
...
int blt_expansion(char** args, FILE* output) {
    ...
    // slt 1 if less than; otherwise 0.
...
int bgt_expansion(char** args, FILE* output) {
    int num_instruct = 0;
    // Same as blt, just flip the registers when slt.
    fprintf(output, "slt $at, %s, %s\n", args[1], args[0]);
    num_instruct++;
...
int swpr_expansion(char** args, FILE* output) {
    ...
    // Move 1st value (stored in $at) to 2nd. 
